...
const { connector } = require('../lib/app');
...
describe('utils module', function () {
    ...
    describe('getImageStream', function () {
        ...
        it('should send a regular GET request to fetch the image', function (done) {
            ...
            sinon.stub(request, 'get', (contentUrl, options) => {
        ...
        it('should throw if it fails to get an access token', function () {
            const attachment = {
                contentUrl: 'https://apis.skype.com/v2/attachments/foo',
                contentType: 'image/jpeg'
            }
            sinon.stub(connector, 'getAccessToken', (callback) => {
                (<any>connector.getAccessToken).restore();
                callback(new Error('Failed to get access token'));
            });

            expect(getImageStream.bind(null, attachment)).to.throw('Failed to get access token');
        });
        ...
        it('should send an authorized GET request to fetch the image from Skype', function (done) {
            ...
            sinon.stub(connector, 'getAccessToken', (callback) => {                
                callback(null, 'some_access_token');
            });
